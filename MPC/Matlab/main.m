...
state = [x y phi v wz].*1.00000000000000001;
...
while t <= T
    ...
    if mod(i-1,Control_Period/dt)==0
        ...
        [wz_command, v_command, delta_f_command, idx_target] = MPC_add_FeedForwardControl1(RefTraj, state_measured, L, t);
    end
    
    state = UpdateRobotState(wz_command, v_command, delta_f_command, state, dt,L);
